  ...
  return <header className="sticky top-0 z-50 w-full">
      ...
      <div className="bg-primary text-primary-foreground">
        <div className="container flex items-center justify-between py-3">
          ...
          <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-primary/80 md:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label={mobileOpen ? "Close navigation menu" : "Open navigation menu"}>
      ...
      {mobileOpen && <nav className="md:hidden bg-primary border-t border-primary-foreground/20" aria-label="Mobile navigation">
          <ul className="container py-2 space-y-1">
            {navLinks.map(link => <li key={link.href}>
                <a href={link.href} className="block px-4 py-3 text-sm font-medium text-primary-foreground hover:bg-primary-foreground/10 rounded transition-colors font-sans" onClick={() => setMobileOpen(false)}>
                  {link.label}
                </a>
              </li>)}
          </ul>
